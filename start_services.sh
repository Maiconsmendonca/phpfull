...
# Função para iniciar ou finalizar serviços
start_services_prompt() {
    # Pergunta ao usuário se deseja iniciar um novo serviço
    read -rp "Deseja iniciar um novo serviço PHP? (s/n): " confirm
    if [[ $confirm == "s" ]]; then
